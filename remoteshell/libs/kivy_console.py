        ...
        def run_cmd(*l):
            ...
            if len(cmd) > 0:
                ...
                try:
                    ...
                    plat = platform
                    # print(txt, '<<')
                    while len(txt) > 0 and self.command_status == 'started':
                        # skip flush on android
                        if plat[0] != 'a':
                            popen_stdout_flush()
                        add_to_cache(txt.decode('utf8'))
                        Logger.debug(txt.decode('utf8'))
                        txt = popen_stdout_r()
...
class std_in_out(object):
    ...
    def __init__(self, obj, mode='stdout'):
        ...
        import threading
        threading.Thread(target=self.read_from_in_pipe).start()
        self.textcache = None
